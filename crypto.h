...
public:
    QByteArray generateAESKey(int size = 32); // 32 bytes = 256-bit
    QByteArray encryptAES(const QByteArray &data, const QByteArray &key, QByteArray &iv);
    QByteArray decryptAES(const QByteArray &cipher, const QByteArray &key, const QByteArray &iv);
};
